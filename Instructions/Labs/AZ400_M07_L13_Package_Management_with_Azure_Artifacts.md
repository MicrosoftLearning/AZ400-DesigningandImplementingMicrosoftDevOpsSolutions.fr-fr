...
1. Dans le volet **Créer un flux**, dans la zone de texte **Nom**, tapez **`eShopOnWebShared`** Dans la section **Visibilité**, sélectionnez Personnes spécifiques, puis, dans la section **Étendue**, sélectionnez l’option **Project:eShopOnWeb**, laissez les autres paramètres avec leurs valeurs par défaut, puis cliquez sur **Créer**.
...
1. Dans la fenêtre **Administrateur : Windows PowerShell**, accédez au dossier eShopOnWeb.Shared en exécutant la commande suivante :
   ...
   > **Remarque** : Le dossier **eShopOnWeb.Shared** est l’emplacement du fichier **eShopOnWeb.Shared.csproj**. Si vous avez choisi un autre emplacement ou un autre nom de projet, accédez à cet emplacement à la place.
...
1. Exécutez ce qui suit pour créer un fichier **.nupkg** à partir du projet (remplacez la valeur de l’espace réservé `XXXXXX` par une chaîne unique).
   ...
   > **Remarque** : dotnet pack génère un package minimal en fonction des informations qu’il est en mesure d’identifier à partir du projet. L’argument `-p:PackageId=eShopOnWeb-XXXXXX.Shared` vous permet de créer un package avec un nom spécifique au lieu d'utiliser le nom contenu dans le projet. Par exemple, si vous remplacez la chaîne `12345` par l’espace réservé `XXXXXX`, le nom du package est **eShopOnWeb-12345.Shared.1.0.0.nupkg**. Le numéro de version a été récupéré de l’assembly.
...
1. Exécutez ce qui suit pour publier le package sur le flux **eShopOnWebShared**. Remplacez la source par l’URL que vous avez copiée précédemment à partir de l’URL **source** de Visual Studio `https://pkgs.dev.azure.com/Azure-DevOps-Org-Name/_packaging/eShopOnWebShared/nuget/v3/index.json`.
   ...
   > **Remarque** : si l’invite n’apparaît pas ou si vous recevez l’avertissement : **Le fournisseur d’informations d’identification du plug-in n’a pas pu acquérir d’informations d’identification. L’authentification peut nécessiter une action manuelle. Envisagez de réexécuter la commande avec --interactive pour `dotnet`, /p :NuGetInteractive=true pour MSBuild ou de supprimer le commutateur -NonInteractive pour NuGet**, vous pouvez ajouter le paramètre **--interactive** à la commande.
...
Lorsque vous avez créé le flux de packages Azure DevOps Artifacts, celui-ci permet, par conception, des **sources en amont**, comme nuget.org dans l’exemple dotnet, qui apporte le package Newtonsoft.Json depuis son emplacement d’hébergement. Il s’agit d’une pratique courante pour éviter la duplication des packages et pour vous assurer que la dernière version est toujours utilisée.
