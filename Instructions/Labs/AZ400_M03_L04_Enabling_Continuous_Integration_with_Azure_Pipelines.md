...
2. Cliquez sur le bouton **Créer un pipeline** ou **Nouveau pipeline**.
...
6. Sélectionnez le fichier **/.ado/eshoponweb-ci-pr.yml**, puis cliquez sur **Continuer**.
...
7. Cliquez sur le bouton **Enregistrer** pour enregistrer la définition de pipeline.
8. Votre pipeline est nommé en fonction du nom du projet. **Renommons**-le pour mieux l’identifier. Accédez à **Pipelines > Pipelines**, puis cliquez sur le pipeline qui vient d’être créé. Cliquez sur les points de suspension, puis sur l’option **Renommer/Supprimer**. Nommez-le **eshoponweb-ci-pr**, puis cliquez sur **Enregistrer**.

#### Tâche 2 : ajouter des stratégies de branche

Dans cette tâche, vous allez ajouter des stratégies à la branche principale et autoriser uniquement les modifications à l’aide de demandes de tirage (pull request) qui sont conformes aux stratégies définies. Vous souhaitez vous assurer que les modifications apportées à une branche sont vérifiées avant d’être fusionnées.
...
5. Sous l’onglet **principal** des paramètres du référentiel, dans la section **Validation de build**, cliquez sur **+** (Ajouter une nouvelle stratégie de build), dans la liste **Pipeline de build**, sélectionnez **eshoponweb-ci-pr**, puis cliquez sur **Enregistrer**.
...
2. Créez une branche nommée **Feature01** basée sur la branche **principale**.
3. Cliquez sur **Feature01**, puis accédez au fichier **/eShopOnWeb/src/Web/Program.cs** dans le cadre de la branche **Feature01**.
4. Cliquez sur le bouton **Modifier** dans le coin supérieur droit.
5. Apportez la modification suivante sur la première ligne :

    ```csharp
    // Testing my PR
    ```

6. Cliquez sur **Valider > Valider** (laissez le message de validation par défaut).
7. Un message s’affiche pour vous proposer de créer une demande de tirage (car votre branche **Feature01** contient désormais des modifications supplémentaires par rapport à la branche **principale**). Cliquez sur **Créer une demande de tirage**.
8. Sous l’onglet **Nouvelle demande de tirage (pull request)**, conservez les valeurs par défaut, puis cliquez sur **Créer**.
9. La demande de tirage affiche certaines exigences en attente en fonction des stratégies appliquées à la branche **principale** cible.
...
10. Une fois toutes les validations réussies, dans le coin supérieur droit, cliquez sur **Approuver**. À présent, dans la liste déroulante **Définir la saisie semi-automatique**, vous pouvez cliquer sur **Terminer**.
11. Sous l’onglet **Achever la demande de tirage**, cliquez sur **Terminer la fusion**.
...
2. À présent, vous devez remplacer les lignes **# trigger:** et **# - main** par le code suivant :
...
3. Cliquez sur le bouton  **Enregistrer** (et non **Enregistrer et exécuter**) pour enregistrer la définition du pipeline.
4. Sélectionnez **Créer une branche pour cette validation**.
5. Conservez le nom de branche par défaut et laissez l’option **Démarrer une demande de tirage** cochée.
6. Dans le menu Paramètres, cliquez sur **Enregistrer**
7. Votre pipeline est nommé en fonction du nom du projet. **Renommons**-le pour mieux l’identifier. Accédez à **Pipelines > Pipelines**, puis cliquez sur le pipeline qui vient d’être créé. Cliquez sur les points de suspension, puis sur l’option **Renommer/Supprimer**. Nommez-le **eshoponweb-ci**, puis cliquez sur **Enregistrer**.
8. Accédez à **Repos > Demandes de tirage**.
9. Cliquez sur la demande de tirage **« Mettre à jour eshoponweb-ci.yml pour Azure Pipelines »**. 
10. Une fois toutes les validations réussies, dans le coin supérieur droit, cliquez sur **Approuver**. Vous pouvez maintenant cliquer sur **Terminer**.
11. Dans l’onglet **Terminer la demande de tirage**, cliquez sur **Terminer la fusion**.
...
2. Créez une branche nommée **Feature02** basée sur la branche **principale**.
3. Cliquez sur la nouvelle branche **Feature02**.
4. Accédez au fichier **/eShopOnWeb/src/Web/Program.cs**, puis cliquez sur **Modifier** en haut à droite.
5. Supprimez la première ligne :
...
6. Cliquez sur **Valider > Valider** (laissez le message de validation par défaut).
7. Un message proposant de créer une demande de tirage (pull request) s’affiche (car votre branche **Feature02** est désormais en avance dans les modifications par rapport à la branche **principale**).
8. Cliquez sur **Créer une demande de tirage**.
9. Sous l’onglet **Nouvelle demande de tirage (pull request)**, conservez les valeurs par défaut, puis cliquez sur **Créer**.
10. La demande de tirage affiche certaines exigences en attente en fonction des stratégies appliquées à la branche **principale** cible.
11. Une fois toutes les validations réussies, dans le coin supérieur droit, cliquez sur **Approuver**. À présent, dans la liste déroulante **Définir la saisie semi-automatique**, vous pouvez cliquer sur **Terminer**.
12. Dans l’onglet **Terminer la demande de tirage**, cliquez sur **Terminer la fusion**.
13. Revenez à **Pipelines > Pipelines**. Vous remarquerez que la build **eshoponweb-ci** a été déclenchée automatiquement après la fusion du code.
14. Cliquez sur la build **eshoponweb-ci**, puis sélectionnez la dernière exécution.
15. Une fois l’exécution réussie, cliquez sur **Associé >Publié** pour vérifier les artefacts publiés :
